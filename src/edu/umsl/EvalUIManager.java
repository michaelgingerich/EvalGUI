...
class EvalTableModel
{

    static final EvalTableModel EVAL_TABLE_DATA = new EvalTableModel();
...
class EvalTableImplementation extends AbstractTableModel
{

    static EvalTableImplementation EVAL_TABLE_MGR;
    ...
    {
        return EvalTableModel.EVAL_TABLE_DATA.getEvalTableWidth();
    }

    @Override
    public String getColumnName(int col)
    {
        return EvalTableModel.EVAL_TABLE_DATA.getEvalTableColName(col);
    }

    @Override
    public int getRowCount()
    {
        return EvalTableModel.EVAL_TABLE_DATA.getEvalTableLength();
    }

    @Override
    public Object getValueAt(int row, int col)
    {
        return EvalTableModel.EVAL_TABLE_DATA.getEvalTableElement(row, col);
    }
...
public class EvalUIManager extends JFrame implements ItemListener
{
    ...
    {
        EVAL_UI_MANAGER.bootstrapDB();
        EVAL_UI_MANAGER.bootstrapEvalInputUI();
    }
    ...
    private void bootstrapEvalInputUI()
    {
        int leftPadding = 5;
...
//        modifyRecordButton.addActionListener(actionListenerInstance);
    ...
    private void bootstrapEvalSummaryUI()
    {
        EvalTableImplementation.EVAL_TABLE_MGR = new EvalTableImplementation();
        summaryTable = new JTable(EvalTableImplementation.EVAL_TABLE_MGR);
        summaryTable.getModel().addTableModelListener(new TableModelListener()
        {
            ...
            public void tableChanged(TableModelEvent e)
            {
                ...
                int evalTableWidth = EvalTableModel.EVAL_TABLE_DATA.getEvalTableWidth();
                for (int col = 0; col < evalTableWidth; col++)
                {
                    EvalTableImplementation.EVAL_TABLE_MGR.setValueAt(EvalTableModel.EVAL_TABLE_DATA.getEvalTableElement(row, col), row, col);
                }
            }
        });
    ...
    private void insertEvals()
    {
        try
        {
            ...
            boolean isInitialized = EvalTableModel.EVAL_TABLE_DATA.isInitialized();
            ...
            {
                evalTableLength = 1;
                EvalTableModel.EVAL_TABLE_DATA.setEvalTableLength(evalTableLength);
                evalTableElements = EvalTableModel.EVAL_TABLE_DATA.getEvalTableElements();
            }
            else
            {
                evalTableLength = EvalTableModel.EVAL_TABLE_DATA.getEvalTableLength() + 1;
                evalTableElements = EvalTableModel.EVAL_TABLE_DATA.getEvalTableElements();
                EvalTableModel.EVAL_TABLE_DATA.setEvalTableLength(evalTableLength);
                //Copy contents
                for (int row = 0; row < evalTableElements.length; row++)
                {
                    for (int col = 0; col < evalTableWidth; col++)
                    {
                        EvalTableModel.EVAL_TABLE_DATA.setEvalTableElement(row, col, evalTableElements[row][col]);
                    }
                }
            }
            ...
            for (int col = 0; col < evalTableWidth; col++)
            {
                EvalTableModel.EVAL_TABLE_DATA.setEvalTableElement(newRowIndex, col, newRow[col]);
            }
            if (!isInitialized)
            {
                bootstrapEvalSummaryUI();
                calcEvalTableColPrefWidth();
                EvalTableModel.EVAL_TABLE_DATA.setIsInitialized(true);
                teamsSummaryButton.setEnabled(true);
            }
            else
            {
                EvalTableImplementation.EVAL_TABLE_MGR.fireTableRowsInserted(newRowIndex, newRowIndex);
            }
            calcEvalTableColPrefWidth();
        }
    ...
    private void selectEvals()
    {
        try
        {
            ...
            if (evalTableLength > 0)
            {
                ...
                {
                    EvalTableModel.EVAL_TABLE_DATA.setEvalTableElement(row, col++, resultSet.getString("TEAMNAME"));
                    EvalTableModel.EVAL_TABLE_DATA.setEvalTableElement(row, col++, resultSet.getInt("TECH"));
                    EvalTableModel.EVAL_TABLE_DATA.setEvalTableElement(row, col++, resultSet.getInt("USEFUL"));
                    EvalTableModel.EVAL_TABLE_DATA.setEvalTableElement(row, col++, resultSet.getInt("CLARITY"));
                    EvalTableModel.EVAL_TABLE_DATA.setEvalTableElement(row, col++, resultSet.getInt("OVERALL"));
                    EvalTableModel.EVAL_TABLE_DATA.setEvalTableElement(row, col, resultSet.getString("COMMENTS"));
                    row++;
                    col = 0;
                }
                bootstrapEvalSummaryUI();
